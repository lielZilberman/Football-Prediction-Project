...
feature_table = en_merge_data.iloc[:, :28]

# Team, Home Goals Score, Away Goals Score, Attack Strength (lines 75, 76), Home Goals Conceded, Away Goals Conceded,
# Defensive Strength (lines 77, 78)
...
feature_table = feature_table[['HomeTeam', 'AwayTeam', 'FTR', 'HST', 'AST', 'B365H', 'B365D']]
f_HAS = []
f_HDS = []
f_AAS = []
f_ADS = []
...
feat_table = feat_table[
    ['HomeTeam', 'AwayTeam', 'FTR', 'FTHG', 'FTAG', 'HS', 'AS', 'HST', 'AST', 'HC', 'AC', 'Referee', 'B365H', 'B365D', 'B365A']]
# Adding next week fixtures
new_fixtures = pd.DataFrame(
    [['Tottenham', 'Man United', 'D', 0, 0, 0, 0, 0, 0, 0, 0, 'J Brooks', 2, 2, 1.9],
     ['Leeds', 'Tottenham', 'D', 0, 0, 0, 0, 0, 0, 0, 0, 'J Brooks', 3, 4, 1.8],
     ['Southampton', 'Liverpool', 'D', 0, 0, 0, 0, 0, 0, 0, 0, 'J Brooks', 4, 2.2, 1.2]],
...
num_games = feat_table.shape[0] - 8
# print(num_games)
# print(feature_table)
v_split = 450
...
X_train = feat_table.loc[0:n_games, ['HS', 'AS', 'HST', 'AST', 'pastGoalDiff', 'HAS', 'HDS', 'AAS', 'ADS', 'B365H','B365D', 'B365A']]
y_train = feat_table.loc[0:n_games, ['Result']]
X_test = feat_table.loc[n_games:num_games - 1,
         ['HS', 'AS', 'HST', 'AST', 'pastGoalDiff', 'HAS', 'HDS', 'AAS', 'ADS', 'B365H', 'B365D', 'B365A']]
y_test = feat_table.loc[n_games:num_games - 1, ['Result']]
X_predict = feat_table[['HS', 'AS', 'HST', 'AST', 'pastGoalDiff', 'HAS', 'HDS', 'AAS', 'ADS', 'B365H', 'B365D', 'B365A']].loc[
            num_games:]
rfm = RandomForestClassifier(n_estimators=100, criterion='entropy')
mlp = MLPClassifier(max_iter=1000, activation='logistic', hidden_layer_sizes=(100, 100))
lgr = LogisticRegression(max_iter=1000)
svc = SVC()

rfm.fit(X_train, np.ravel(y_train))
res = rfm.predict(X_test)
print("rfm: ", accuracy_score(y_test, res))
# # KNN
...
print("mlp: ", accuracy_score(y_test, res))

lgr.fit(X_train, np.ravel(y_train))
print("lgr: ", accuracy_score(y_test, lgr.predict(X_test)))
...
print("xgb: ", max_XGB_e, max_XGB_ind)
...
y_pred_lgr = lgr.predict(X_predict)
y_pred_svc = svc.predict(X_predict)

this_week = feat_table[['HomeTeam', 'AwayTeam', 'Referee']][num_games:]
print(feat_table)
...
this_week['Result_lgr'] = y_pred_lgr
...
this_week["Res_lgr"] = this_week.apply(lambda row: transformResultBack(row, "Result_lgr"), axis=1)
...
this_week.drop(["Result_knn", "Result_rfm", "Result_mlp", "Result_lgr", "Result_svc"], axis=1, inplace=True)
